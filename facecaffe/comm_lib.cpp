...
void process_boxes(std::vector<FaceBox>& input, int img_h, int img_w, std::vector<FaceBox>& rects,float nms_threshold)
{

	nms_boxes(input,nms_threshold,NMS_UNION,rects);//nms_threshold default & orignal value is 0.7 
...
void cal_landmark(std::vector<FaceBox>& box_list)
{
	for(unsigned int i=0;i<box_list.size();i++)
	{
		...
		for(int j=0;j<5;j++)
		{
			box.landmark[j]=box.x0+w*box.landmark[j]-1;
			box.landmark[j+5]=box.y0+h*box.landmark[j+5]-1;
		}
